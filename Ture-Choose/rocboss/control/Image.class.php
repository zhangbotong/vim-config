...
class Image
{
    ...
    {
        return 'uploads/avatars/' . $uid . '/';
    }
    public static function getAvatarURL($uid, $size = 100)
    {
        $avatarPath = self::avatarPath($uid) . $size . '.png';
        return $avatarPath;
    }
    public static function CreatDefaultAvatar($userId)
    {
        ...
        $defaultBigAvatar   = "uploads/avatars/0/avatar_b.png";
        $defaultSmallAvatar = "uploads/avatars/0/avatar_s.png";
    ...
    public static function RandomCode($width = 120, $height = 38, $verifyName = 'identifying_code')
    {
        ...
        $_SESSION[$verifyName] = md5(strtolower($verifyNum) . $GLOBALS['roc_config']['secure_key']);
        $im                    = imagecreate($width, $height);
        imagecolorallocatealpha($im, 255, 255, 255, 100);
        $color = imagecolorallocate($im, rand(0, 230), rand(0, 230), rand(0, 230));
        imagettftext($im, 16, 0, 5, 30, $color, 'themes/rocboss/images/OctemberScript.ttf', $verifyNum);
        Image::output($im, 'png');
    }
