...
class Secret
{
    ...
    public static function encrypt($txt, $key = 'abcd9667676effff')
    {
        $s = urlencode(base64_encode(self::xxtea_encrypt($txt, $key)));
        $s = str_replace('%2F', '%252F', $s);
        return $s;
    }

    public static function decrypt($txt, $key = 'abcd9667676effff')
    {
